...
def sdHexPrism(p, h):
    ...
    d = float2(length(p.xy - float2(clamp(p.x, -k.z * h.x, k.z * h.x), h.x)) * sign(p.y - h.x), p.z - h.y)
...
def sdCylinder1(p, a, b, r):
    ...
    x2 = x * x
    y2 = y * y * baba
    d = ite((max(x, y) < 0.0), -min(x2, y2), (ite((x > 0.0), x2, 0.0) + ite((y > 0.0), y2, 0.0)))
...
def sdRhombus(p, la, lb, h, ra):
    ...
    q = float2(
        length(p.xz - 0.5 * b * float2(1.0 - f, 1.0 + f)) * sign(p.x * b.y + p.z * b.x - b.x * b.y) - ra, p.y - h
    )
...
def sdHorseshoe(p, c, r, le, w):
    ...
    p = float3(float2(ite((p.y > 0.0 or p.x > 0.0), p.x, line_dist * sign(-c.x)), ite((p.x > 0.0), p.y, l)), p.z)
...
def map(pos):
    ...
    if sdBox(pos - float3(0.0, 0.3, -1.0), float3(0.35, 0.3, 2.5)) < res.x:
        res = opU(
            res,
            float2(
                sdCappedTorus((pos - float3(0.0, 0.30, 1.0)) * float3(1, -1, 1), float2(0.866025, -0.5), 0.25, 0.05),
                25.0,
            ),
        )
    ...
    res = opU(res, float2(sdCappedCone0(pos - float3(0.0, 0.25, -2.0), 0.25, 0.25, 0.1), 13.67))
    ...
    res = opU(
        res, float2(sdCapsule(pos - float3(1.0, 0.00, -1.0), float3(-0.1, 0.1, -0.1), float3(0.2, 0.4, 0.2), 0.1), 31.9)
    )
    res = opU(res, float2(sdCylinder0(pos - float3(1.0, 0.25, -2.0), float2(0.15, 0.25)), 8.0))
    ...
    res = opU(
        res,
        float2(
            sdHorseshoe(pos - float3(-1.0, 0.25, 1.0), float2(cos(1.3), sin(1.3)), 0.2, 0.3, float2(0.03, 0.08)), 11.5
        ),
    )
    ...
    res = opU(
        res,
        float2(sdCylinder1(pos - float3(2.0, 0.14, -2.0), float3(0.1, -0.1, 0.0), float3(-0.2, 0.35, 0.1), 0.08), 31.2),
    )
    res = opU(
        res,
        float2(
            sdCappedCone1(pos - float3(2.0, 0.09, -1.0), float3(0.1, 0.0, 0.0), float3(-0.2, 0.40, 0.1), 0.15, 0.05),
            46.1,
        ),
    )
    res = opU(
        res,
        float2(
            sdRoundCone1(pos - float3(2.0, 0.15, 0.0), float3(0.1, 0.0, 0.0), float3(-0.1, 0.35, 0.1), 0.15, 0.05), 51.7
        ),
    )
    res = opU(res, float2(sdRoundCone0(pos - float3(2.0, 0.20, 1.0), 0.2, 0.1, 0.3), 37.0))
...
def render(ro, rd, rdx, rdy):
    ...
    if m > -0.5:
        ...
        # back
        dif = clamp(dot(nor, normalize(float3(0.5, 0.0, 0.6))), 0.0, 1.0) * clamp(1.0 - pos.y, 0.0, 1.0)
        dif *= occ
...
def kernel(mouse, time, img):
    ...
    ro = ta + float3(5.5 * cos(0.1 * time + 7.0 * mo.x), 2.2, 5.5 * sin(0.1 * time + 7.0 * mo.x))
    ...
    tot += col
    img.write(dispatch_id().xy, float4(tot, 1.0))
...
def downsample_tex(image, out_image):
    ...
    while value > 1:
        ...
        if all(local_coord < uint2(next_value)):
            ...
            col = (
                arr[value * last_coord.y + last_coord.x]
    ...
    if all(local_coord == uint2(0)):
        out_image.write(block_id().xy, float4(col, 1.0))
...
time = 0.0
mouse_pos = float2(0.0)
mouse_movement = float2(0.0)
last_mouse = float2(0.0)
while gui.running():
    # left mouse
