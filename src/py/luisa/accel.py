...
class Accel:
    def __init__(
        self,
        hint: lcapi.AccelUsageHint = lcapi.AccelUsageHint.FAST_BUILD,
    ...
    def add(
        ...
        allow_compact: bool = False,
        allow_update: bool = False,
        visibility_mask: int = -1,
        opaque: bool = True,
        user_id: int = 0,
    ):
        self._accel.emplace_back(
            vertex_buffer.handle,
            0,
            vertex_buffer.bytesize,
            to_lctype(vertex_buffer.dtype).size(),
            triangle_buffer.handle,
            0,
            triangle_buffer.bytesize,
            transform,
            allow_compact,
            allow_update,
            visibility_mask,
    ...
    def add_procedural(
        ...
        self._accel.emplace_procedural(
            aabb_buffer.handle,
            aabb_start_index,
            var_aabb_count,
            transform,
    ...
    def trace_closest(self, ray, vis_mask):
        ...
        expr = lcapi.builder().call(
            to_lctype(TriangleHit), lcapi.CallOp.RAY_TRACING_TRACE_CLOSEST, [self.expr, ray.expr, vis_mask.expr]
        )
    ...
    def trace_closest_cullback(self, ray, vis_mask):
        ...
        expr = lcapi.builder().call(
            to_lctype(TriangleHit),
            lcapi.CallOp.RAY_TRACING_TRACE_CLOSEST_CULL_BACKFACE,
            [self.expr, ray.expr, vis_mask.expr],
        )
        return TriangleHit, expr
    ...
    def trace_any(self, ray, vis_mask):
        ...
        expr = lcapi.builder().call(
            to_lctype(bool), lcapi.CallOp.RAY_TRACING_TRACE_ANY, [self.expr, ray.expr, vis_mask.expr]
        )
        return bool, expr
    ...
    def trace_any_cullback(self, ray, vis_mask):
        ...
        expr = lcapi.builder().call(
            to_lctype(bool), lcapi.CallOp.RAY_TRACING_TRACE_ANY_CULL_BACKFACE, [self.expr, ray.expr, vis_mask.expr]
        )
        return bool, expr
    ...
    def instance_transform(self, index):
        ...
        expr = lcapi.builder().call(
            to_lctype(float4x4), lcapi.CallOp.RAY_TRACING_INSTANCE_TRANSFORM, [self.expr, index.expr]
        )
    ...
    def visibility_mask(self, index):
        ...
        expr = lcapi.builder().call(
            to_lctype(uint), lcapi.CallOp.RAY_TRACING_INSTANCE_VISIBILITY_MASK, [self.expr, index.expr]
        )
        return uint, expr
    ...
    def set_instance_transform(self, index, transform):
        ...
        expr = lcapi.builder().call(
            to_lctype(float4x4),
            lcapi.CallOp.RAY_TRACING_SET_INSTANCE_TRANSFORM,
            [self.expr, index.expr, transform.expr],
        )
        return float4x4, expr
    ...
    def set_instance_visibility(self, index, visibility_mask):
        ...
        expr = lcapi.builder().call(
            lcapi.CallOp.RAY_TRACING_SET_INSTANCE_VISIBILITY, [self.expr, index.expr, visibility_mask.expr]
        )
        return None, expr
    ...
    def set_instance_opacity(self, index, opacity):
        ...
        expr = lcapi.builder().call(
            lcapi.CallOp.RAY_TRACING_SET_INSTANCE_OPACITY, [self.expr, index.expr, opacity.expr]
        )
        return None, expr
    ...
    def set_instance_user_id(self, index, opacity):
        ...
        expr = lcapi.builder().call(
            lcapi.CallOp.RAY_TRACING_SET_INSTANCE_USER_ID, [self.expr, index.expr, opacity.expr]
        )
        return None, expr
    ...
    def query_all(self, ray, vis_mask):
        ...
        expr = lcapi.builder().call(
            to_lctype(rayQueryAllType), lcapi.CallOp.RAY_TRACING_QUERY_ALL, [self.expr, ray.expr, vis_mask.expr]
        )
    ...
    def query_any(self, ray, vis_mask: int):
        ...
        expr = lcapi.builder().call(
            to_lctype(rayQueryAnyType), lcapi.CallOp.RAY_TRACING_QUERY_ANY, [self.expr, ray.expr, vis_mask.expr]
        )
