    ...
    if arch == "arm64":
        subprocess.run(
            [
                ...
                "--math-lib=fast",
                "-woff",
                "--opt=enable-ldst-vectorizer",
                "--opt=enable-slp-vectorizer",
                "--target=neon-i32x8",
            ]
        )
    else:
...
def patch_llvm_ir(src_file: str, output_file: str, preserved_functions: list):
    with open(src_file) as f:
        src = f.readlines()
    patched_lines = [
        line.replace("define ", "define private ")
        if (line.startswith("define ") and not any(line.startswith(f"define void @{f}(") for f in preserved_functions))
        else line
        for line in src
    ]
    with open(output_file, "w") as f:
...
def embed_bitcode_into_c_hex_array(bc_file: str, output_file: str, array_name: str):
    with open(bc_file, "rb") as f:
        bc = f.read()
    with open(output_file, "w") as f:
        f.write("// This file is generated by generate_ispc_tex_compress.py\n")
