...
def print_help():
    ...
    print(
        "  --toolchain | -t [toolchain]      Configure toolchain (effective only",
    ...
    print("      Dependencies:")
    print(
        "          all                Install all dependencies required by CMake and XMake builds",
        "(" + ", ".join(ALL_DEPENDENCIES) + ")",
    )
    print(
        "          all-cmake          Install all dependencies required by CMake builds",
        "(" + ", ".join(ALL_CMAKE_DEPENDENCIES) + ")",
    )
    print(
        "          all-xmake          Install all dependencies required by XMake builds",
...
def build_system_config_args_cmake(config: dict, mode: str, toolchain: str, toolchain_version: int):
    ...
    cmake_mode = {"debug": "Debug", "release": "Release", "reldbg": "RelWithDebInfo"}
...
def build_system_config_args_xmake(config: dict, mode: str, toolchain: str, toolchain_version: int):
    ...
    xmake_mode = {"debug": "debug", "release": "release", "reldbg": "releasedbg"}
    args.append("-m")
...
def init_submodule():
    ...
    git_submodules = {git_submodules[sec]["path"]: git_submodules[sec]["url"] for sec in git_submodules.sections()}
...
def main(args: list[str]):
    ...
    if run_config or run_build:
        ...
        build_config["environment"] = {
            k: v for k, v in os.environ.items() if k not in environ_backup or environ_backup[k] != v
        }
